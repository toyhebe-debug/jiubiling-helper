import HKProcurement from './hk-procurement';
import {pageFromHash} from '@/lib/hk';
import PreparationPanel from './preparation-panel';
import {createPreparation} from '@/lib/preparation';
import {apiFetch} from '@/lib/api';
"use client";
import {useCallback,useEffect,useRef,useState} from 'react';
import {Heart,ShoppingBasket,Milk,Leaf,CalendarDays,Check,Plus,ArrowRight,ArrowLeft,Settings2,RefreshCw,Download,CheckCheck,WifiOff,X,NotebookPen,BriefcaseBusiness,House} from 'lucide-react';
...
export default function FamilyApp({onLogout}:{onLogout:()=>Promise<void>}){
 ...
 const [saving,setSaving]=useState(false),savingRef=useRef(false),[editor,setEditor]=useState<Editor|null>(null),[tab,setTab]=useState(()=>pageFromHash(location.hash));
 useEffect(()=>{const change=()=>setTab(pageFromHash(location.hash));window.addEventListener('hashchange',change);return()=>window.removeEventListener('hashchange',change)},[]);
 function openHK(){location.hash='hk';setTab('hk')}
 function closeHK(){history.replaceState(null,'',location.pathname+location.search);setTab('today')}
 ...
 {tab==='hk'?<HKProcurement trip={data.hkTrip} version={snapshot.version} saving={saving} onBack={closeHK} onSave={async(hkTrip,version)=>!!await save({...ref.current!.data,hkTrip},version)}/>:<><div className="greeting"><p className="eyebrow">{pretty(date)} · {weekday(date)}</p><h1>缺什么记什么</h1><p>{pregnancy(data.dueDate,date)} <span className="dot">·</span> 预产期 {pretty(data.dueDate)}</p></div>
 <button className="hk-entry" onClick={openHK}><span><BriefcaseBusiness size={20}/><strong>香港采购</strong><small>清单、路线与门店</small></span><ArrowRight size={18}/></button>
 <Tabs value={tab} onValueChange={setTab}><TabsList className="main-tabs"><TabsTrigger value="today"><ShoppingBasket size={17}/> 日常备货</TabsTrigger><TabsTrigger value="hospital"><BriefcaseBusiness size={17}/> 待产包</TabsTrigger><TabsTrigger value="home"><House size={17}/> 母婴用品</TabsTrigger><TabsTrigger value="visit"><CalendarDays size={17}/> 产检</TabsTrigger></TabsList>
